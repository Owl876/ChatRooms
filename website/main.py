...
from schema import UserAuthorizate, Token, ChatRequest, LogoutMessage
...
async def register(request: Request, db: Session = Depends(get_db), email: str = Form(None), password: str = Form(None)):
    """Обработка регистрации пользователя (GET и POST запросы)."""
...
async def authorizate(request: Request, response: Response, db: Session = Depends(get_db), email: str = Form(None), password: str = Form(None), is_authorizate: bool = Depends(is_user_authorizate)):
    ...
    # Обработка POST-запроса
        ...
        form_data = UserAuthorizate(email=email, password=password)
